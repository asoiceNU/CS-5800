...
def interactive():
    ...
    print("Red-Black Tree built.")
    
    while True:
        command = input("Enter command (insert <x>, search <x>, sort, height, exit): ").strip().lower()
        ...
        if command.startswith('insert'):
            ...
            _, x = command.split()
            x = int(x)
            node = tree.search_tree(tree.root, x)
            if node != tree.TNULL:
                print(f"Found {x}")
            else:
                print(f"{x} not found.")
        ...
        # Print tree height after each operation
        height = tree.tree_height(tree.root)
        print(f"Tree height: {height}\n")
